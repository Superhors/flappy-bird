...
class GameState:
    ...
    def frame_step(self, input_actions):
        ...
        if  self.playery >self.lowerPipes[0]['y']-int(PIPEGAPSIZE/2):
            self.input_action=0
        elif  self.playery <self.lowerPipes[0]['y']-int(PIPEGAPSIZE/2):
            self.input_action=1

        if self.input_action==0:
            self.playerVelY=self.playerFlapAcc
        elif self.input_action==1:
        ...
        if SCREENWIDTH/2-5<self.upperPipes[0]['x']+PIPE_WIDTH<SCREENWIDTH/2:
            self.i+=1
        scoreDigits = [int(x) for x in list(str(self.i))]
